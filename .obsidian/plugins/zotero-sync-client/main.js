...
var require_zotero_sync = __commonJS({
  "node_modules/@retorquere/zotero-sync/index.js"(exports) {
    ...
    var Sync = class {
      ...
      async fetch(url) {
        return await node_fetch_1.default(url, { headers: this.headers });
      }
      async json(url) {
...
var require_json_stringify_pretty_compact = __commonJS({
  "node_modules/json-stringify-pretty-compact/index.js"(exports, module2) {
    "use strict";
    var stringOrChar = /("(?:[^\\"]|\\.)*")|[:,]/g;
  ...
  "node_modules/@retorquere/zotero-sync/json-store.js"(exports) {
    ...
    var Library = class {
      async load(filename) {
        ...
        let name = "";
        try {
          ({ items, collections, version, name } = JSON.parse(await fs2.promises.readFile(this.filename, "utf-8")));
        } catch (err) {
        }
        this.items = items;
...
var ZoteroSyncClientPlugin = class extends import_obsidian.Plugin {
  ...
  generateNoteFilePath(data, template = null) {
    if (!template) {
  ...
  generateNote(data, template = null) {
    if (!template) {
  ...
  getMarker(element) {
    ...
    return `[\u{1F1FF}](zotero://select/library/${kind}/${element.key})`;
  }
